...
**Not yet done**: actually applying 203–207 to a live Postgres and running dynamic tests (concurrency, RLS cross-org denial, idempotent-replay, the full acceptance matrix in section F of the brief). Blocked on a local tooling conflict — both Docker Desktop and Rancher Desktop are installed; the active `docker` CLI context (`npipe:////./pipe/docker_engine`) isn't reaching either engine's running backend. This needs to be resolved (pick one engine) before Phase 5 can produce real dynamic evidence — every line in this document above the word "Not yet done" is a static-analysis and code-reading claim, not a database-verified one, and is reported as such.
