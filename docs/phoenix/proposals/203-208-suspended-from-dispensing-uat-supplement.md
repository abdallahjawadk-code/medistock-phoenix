...
| 206 | `206-…-enforcement-suggestions` | An identical surplus/deficit corridor yields a real suggestion for a free material (positive control) and **none** for a suspended one, on both engines, with lifting restoring it. The two refuse *differently* and the suite asserts that difference rather than assuming they behave alike: the intra-org engine sweeps every material and simply yields nothing for a filtered-out source, while the cross-org engine — asked about one *named* material whose source pool 206 has emptied — refuses outright with `no_eligible_fefo_batch`. No live suggestion exists either way; only the explicit rejection proves the gated call was reached rather than skipped |
