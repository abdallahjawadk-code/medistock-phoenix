-- =============================================================================
-- MediStock Phoenix V2 — Migration 058: Public QR Dosage Form (additive)
-- =============================================================================
-- MANUAL APPLY ONLY. DO NOT use `supabase db push` or any automated runner.
-- Apply via Supabase Dashboard → SQL Editor after reading this file in full.
-- Apply ONLY after migration 057 is confirmed applied and healthy.
--
-- Prerequisites: Migrations 001–057 must already be applied.
--
...
-- ─────────────────────────────────────────────────────────────────────────────
--   The public QR material list (get_public_qr_payload) already returns each
--   material's public-safe display name, unit, effective condition, expiry
--   bucket, quantity, and (guarded) expiry date. It does NOT return the
--   pharmaceutical dosage form, even though operators can enter it and it is
--   stored on item_availability.dosage_form (added in migration 020).
--
--   dosage_form (e.g. "Tablet", "Syrup", "Injection", "أقراص") is descriptive
--   clinical information — analogous to the already-public scientific_name /
--   material name. It is not price, batch, supply, contact, identity, or any
--   internal/operational field. Surfacing it helps the public tell apart two
--   materials that share the same scientific name but differ in form.
--
--   This migration ADDITIVELY adds a single new field, 'dosage_form', to the
--   two per-material item objects in the payload:
--     • the distribution_point branch's items[] objects
--     • the local_item branch's availability[] objects
--   Its value is item_availability.dosage_form verbatim (JSON null when null —
--   no default, no normalization, no translation, no cross-table substitution).
--
-- ─────────────────────────────────────────────────────────────────────────────
-- SOURCE OF TRUTH
-- ─────────────────────────────────────────────────────────────────────────────
--   The function body below is taken verbatim from migration 052 (the latest
--   active definition of get_public_qr_payload). The ONLY semantic differences
--   from migration 052 are the two additive dosage_form entries described above,
--   each threaded through the existing per-branch `derived` subquery exactly the
--   way ia.quantity / ia.expiry_date already are (the outer jsonb_build_object
--   has no `ia` alias in scope — only `derived` — so the value is selected as
--   derived.ia_dosage_form, matching the established pattern). Nothing else is
--   refactored, reordered, renamed, or re-filtered.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- WHAT IS NOT CHANGED
-- ─────────────────────────────────────────────────────────────────────────────
...
--   • LANGUAGE plpgsql / volatility (default VOLATILE) — unchanged.
--   • Grants: unchanged (revoke from authenticated, grant execute to anon + authenticated).
--   • Token resolution, scan counter (last_scanned_at / scan_count), org resolution: unchanged.
--   • warehouse branch: unchanged (no per-material item object there — no dosage_form added).
--   • D1 (avail_select_anon using false — anon has NO direct item_availability SELECT),
--     D4 (distribution_point status='active' guard), D6 (LEFT JOIN + scientific_name
--     fallback), D7 (effective_condition + expiry_bucket), 052 quantity<=0 -> 'missing':
--     all preserved exactly.
--   • expiry_bucket, effective_condition, D2 (expired -> quantity null),
--     D3 (expiry_date only for near_expiry/expired), ordering: unchanged.
--   • No table/column/index changes. No RLS/policy changes. No new grants.
--   • No data mutation. No DROP TABLE, DROP COLUMN, TRUNCATE, destructive DELETE, unsafe CASCADE.
...
-- =============================================================================
-- Replace get_public_qr_payload — additively add dosage_form to the two
-- per-material item objects. Body is otherwise byte-for-byte identical to
-- migration 052's definition. The only additions are marked "058:" below.
-- =============================================================================
...
begin
  ...
  case v_target.target_type
    ...
    when 'warehouse' then
      -- warehouse branch: unchanged from migration 027/028/052 — this branch
      -- only returns point-level item_count, with no per-item object of any
      -- kind, so no dosage_form is added here.
      select jsonb_build_object(
        'ok',              true,
...
begin
  ...
  -- 058: dosage_form added to the JSON output in EXACTLY the two intended
  -- material branches (distribution_point items[] + local_item availability[]).
  -- Count the JSON key occurrences and the source-column references.
  v_dosage_json := (length(v_fn_def) - length(replace(v_fn_def, '''dosage_form'',', ''))) / length('''dosage_form'',');
  assert v_dosage_json = 2,
    'VERIFY FAILED (058): expected exactly 2 ''dosage_form'' JSON keys (distribution_point + local_item), found ' || v_dosage_json;

  -- 058: sourced from item_availability.dosage_form (threaded as ia.dosage_form
  -- in each branch's derived subquery) — exactly twice.
  ...
  assert v_dosage_src = 2,
    'VERIFY FAILED (058): expected exactly 2 ia.dosage_form source references, found ' || v_dosage_src;
  ...
  raise notice 'Migration 058 verification passed: get_public_qr_payload now additively returns dosage_form (from item_availability.dosage_form) in the distribution_point and local_item material objects only. Signature, return type, SECURITY DEFINER, SET search_path, grants, anon RLS (avail_select_anon = false), scan bookkeeping, D4/D6/D7 and the 052 quantity<=0 rule all preserved; no private field added, no destructive/schema/RLS change.';
end $$;
...
--
-- 2. dosage_form present for a distribution_point QR whose rows have a
--    non-null dosage_form:
--    SELECT get_public_qr_payload('<public_id>');
--    Expected: each items[] object now has a "dosage_form" key with the stored
--    value; rows with a null dosage_form show "dosage_form": null.
--
-- 3. Regression: condition / quantity / expiry_date / expiry_bucket fields for
--    the same rows are byte-for-byte identical to before migration 058.
--
-- 4. Disabled QR still blocked:
--    SELECT get_public_qr_payload('<disabled_public_id>');
--    Expected: {"ok": false, "error": "QR_NOT_FOUND_OR_DISABLED"}
--
-- 5. Archived/inactive distribution point still blocked:
