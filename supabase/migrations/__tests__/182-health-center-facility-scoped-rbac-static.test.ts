...
describe('182 registration and shape', () => {
  ...
  // still fails closed.
  it('is followed by exactly 183 through 202, and nothing beyond them', () => {
    ...
    const SUCCESSORS = [
      ...
      // changes no body, grant or search_path, so the facility-scoped RBAC
      // surface 182 owns is untouched. It is now the reviewed ceiling.
      '214_phoenix_central_needs_review_readiness_volatility.sql',
      // C2/M215: the governed Central Needs correction lifecycle — function-only (no table, column, enum,
      // trigger, policy or permission key) and no facility-scoped RBAC SQL, so nothing this suite asserts moves.
      // It is now the reviewed ceiling.
      '215_phoenix_central_needs_governed_correction_lifecycle.sql',
      // C4/M216: Central Needs beneficiary-region persistence — additive (one region version relation with RLS on it
      // alone, guard triggers, one fenced write RPC with the audited M213 -> region conversion, behaviour-only Central
