...
describe('191 · registration and file hygiene', () => {
  ...
  it('is immediately followed by 192 through 202, the new ceiling, and 203 stays absent', () => {
    ...
    const NEXT_7 = '198_phoenix_secdef_search_path_convergence.sql';
    const NEXT_8 = '199_phoenix_command_center_read_contract.sql';
    const NEXT_9 = '200_phoenix_demo_purge_auth_boundary_correction.sql';
    const NEXT_10 = '201_phoenix_organization_archive_dependency_guard.sql';
    const NEXT_11 = '202_phoenix_organization_archive_reciprocal_guard.sql';
    const NEXT_12 = '203_phoenix_material_dispensing_suspension.sql';
    const NEXT_13 = '204_phoenix_dispensing_suspension_enforcement_dispense.sql';
    const NEXT_14 = '205_phoenix_dispensing_suspension_enforcement_fefo.sql';
    ...
    const NEXT_19 = '210_phoenix_central_needs_workflow_rpcs.sql';
    const NEXT_20 = '211_phoenix_central_needs_batch_and_disposition.sql';
    const NEXT_21 = '212_phoenix_central_needs_need_lines.sql';
    // CN-2B / Finding-1 corrective (213): the Central Needs beneficiary-
    // column-mapping layer. It is now the reviewed successor.
    const NEXT_22 = '213_phoenix_central_needs_beneficiary_column_mapping.sql';
    ...
    // search_path change. It adds no topology/scope SQL, so what 191 asserts is
    // unaffected; listed to keep this successor guard exhaustive.
    const NEXT_23 = '214_phoenix_central_needs_review_readiness_volatility.sql';
    ...
    expect(REVIEWED_MIGRATION_FILES.slice(REVIEWED_MIGRATION_FILES.indexOf(NAME) + 1)).toEqual([NEXT, NEXT_2, NEXT_3, NEXT_4, NEXT_5, NEXT_6, NEXT_7, NEXT_8, NEXT_9, NEXT_10, NEXT_11, NEXT_12, NEXT_13, NEXT_14, NEXT_15, NEXT_16, NEXT_17, NEXT_18, NEXT_19, NEXT_20, NEXT_21, NEXT_22, NEXT_23]);
    ...
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^201_/.test(f))).toEqual([NEXT_10]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^202_/.test(f))).toEqual([NEXT_11]);
    ...
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^210_/.test(f))).toEqual([NEXT_19]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^211_/.test(f))).toEqual([NEXT_20]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^212_/.test(f))).toEqual([NEXT_21]);
    ...
    // The next unauthorized number must still fail closed — now 215.
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^215_/.test(f))).toHaveLength(0);
  });
