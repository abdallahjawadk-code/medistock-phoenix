...
describe('Regression: UI screens and unrelated systems untouched', () => {
  ...
  it('AdjustQuantityModal / MovementHistoryModal are unaffected', () => {
    expect(adjustModal).not.toContain('inter_org_exchange_requests');
  ...
  it('no direct supabase.from() call to the new exchange tables exists anywhere in src', () => {
    ...
    [alertsScreen, dashboardScreen, adjustModal, historyModal, service, lifecycleService].forEach(src => {
