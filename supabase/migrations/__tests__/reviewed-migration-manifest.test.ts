...
describe('reviewed migration manifest through 186',()=>{
  ...
  it('pins maximum 186 and next unreviewed 187',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(186);
    expect(getNextUnreviewedMigrationNumber()).toBe(187);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=187;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
