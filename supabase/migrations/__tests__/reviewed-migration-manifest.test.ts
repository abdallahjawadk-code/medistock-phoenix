...
const SYNTH_NEXT='177_unreviewed_test_migration.sql';
const SYNTH_ALT='176_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 176',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(176);
  ...
  it('pins maximum 176 and next unreviewed 177',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(176);
    expect(getNextUnreviewedMigrationNumber()).toBe(177);
    expect(isReviewedMigrationFile('176_phoenix_canonical_outlet_availability_read_model.sql')).toBe(true);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=177;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,176],[150,176],[170,176],[174,176],[175,176],[176,176],[177,177]] as const)
