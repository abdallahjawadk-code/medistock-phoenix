...
const SYNTH_NEXT = '087_unreviewed_test_migration.sql';
const SYNTH_NEXT_ALT = '087_phoenix_some_other_name.sql';
...
describe('3. reviewed maximum derives from the registry', () => {
  it('the current reviewed maximum is 86', () => {
    expect(getMaximumReviewedMigrationNumber()).toBe(86);
  });

  it('the next unreviewed number is 87', () => {
    expect(getNextUnreviewedMigrationNumber()).toBe(87);
  });
...
describe('4. migrations 059–073 registered by exact name; 074 absent', () => {
  ...
  it('registers exactly one each of migrations 074-086 (no alternate names)', () => {
  ...
  it('does not register migration 087 in any form', () => {
    const eightySevens = REVIEWED_MIGRATION_FILES.filter(f => extractMigrationNumber(f) === 87);
    expect(eightySevens).toEqual([]);
  });
});
...
describe('5. approval requires exact filename membership, nothing less', () => {
  it('rejects a synthetic unreviewed migration 087 (the next unreviewed number)', () => {
...
describe('6. derived slices remain exact-filename lists', () => {
  ...
  it('reviewedMigrationFilesAbove(64) contains exactly migrations 065-085', () => {
    expect(reviewedMigrationFilesAbove(64)).toEqual([REAL_065, REAL_066, REAL_067, REAL_068, REAL_069, REAL_070, REAL_071, REAL_072, REAL_073, REAL_074, REAL_075, REAL_076, REAL_077, REAL_078, REAL_079, REAL_080, REAL_081, REAL_082, REAL_083, REAL_084, REAL_085, REAL_086]);
  });

  it('reviewedMigrationFilesAbove(71) contains exactly migrations 072-085', () => {
    expect(reviewedMigrationFilesAbove(71)).toEqual([REAL_072, REAL_073, REAL_074, REAL_075, REAL_076, REAL_077, REAL_078, REAL_079, REAL_080, REAL_081, REAL_082, REAL_083, REAL_084, REAL_085, REAL_086]);
  });

  it('reviewedMigrationFilesAbove(72) contains exactly migrations 073-085', () => {
    expect(reviewedMigrationFilesAbove(72)).toEqual([REAL_073, REAL_074, REAL_075, REAL_076, REAL_077, REAL_078, REAL_079, REAL_080, REAL_081, REAL_082, REAL_083, REAL_084, REAL_085, REAL_086]);
  });

  it('reviewedMigrationFilesAbove(76) contains exactly migrations 077-085', () => {
    expect(reviewedMigrationFilesAbove(76)).toEqual([REAL_077, REAL_078, REAL_079, REAL_080, REAL_081, REAL_082, REAL_083, REAL_084, REAL_085, REAL_086]);
  });

  it('reviewedMigrationFilesAbove(77) contains exactly migrations 078-085', () => {
    expect(reviewedMigrationFilesAbove(77)).toEqual([REAL_078, REAL_079, REAL_080, REAL_081, REAL_082, REAL_083, REAL_084, REAL_085, REAL_086]);
  });

  it('reviewedMigrationFilesAbove(86) is empty (nothing beyond the ceiling)', () => {
    expect(reviewedMigrationFilesAbove(86)).toEqual([]);
  });
