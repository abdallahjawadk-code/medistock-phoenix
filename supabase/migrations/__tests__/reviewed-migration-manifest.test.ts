...
const SYNTH_NEXT='179_unreviewed_test_migration.sql';
const SYNTH_ALT='178_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 178',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(178);
  ...
  it('pins maximum 178 and next unreviewed 179',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(178);
    expect(getNextUnreviewedMigrationNumber()).toBe(179);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=179;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,178],[150,178],[170,178],[174,178],[175,178],[176,178],[177,178],[178,178],[179,179]] as const)
