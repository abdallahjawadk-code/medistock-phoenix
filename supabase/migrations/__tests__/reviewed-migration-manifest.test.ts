...
// C5/M217 is the Central Needs safety convergence (classifier, shared lineage
// helper, NOT VALID source-value CHECK, approval-gate fence; no stock,
// movement, allocation or transfer SQL) and this manifest owns the current
// reviewed ceiling.
// The synthetic unknown boundary moves to 218 and must remain genuinely absent:
// approving 217 authorizes 217 by exact filename and nothing after it.
const SYNTH_NEXT='218_unauthorized_probe.sql';
const SYNTH_ALT='187_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 217',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(217);
  ...
  it('pins maximum 217 and next unreviewed 218',()=>{
    ...
    expect(isReviewedMigrationFile('183_phoenix_emergency_outlet_integrity.sql')).toBe(true);
    expect(isReviewedMigrationFile('184_phoenix_canonical_supply_cycle.sql')).toBe(true);
    expect(isReviewedMigrationFile('185_phoenix_return_quarantine_recall_parity.sql')).toBe(true);
    expect(isReviewedMigrationFile('186_phoenix_correction_reason_code_wrapper_parity.sql')).toBe(true);
    expect(isReviewedMigrationFile('187_phoenix_delegated_operational_access.sql')).toBe(true);
    expect(isReviewedMigrationFile('188_phoenix_public_qr_facility_context.sql')).toBe(true);
    expect(isReviewedMigrationFile('189_phoenix_inter_org_alert_canonical_identity.sql')).toBe(true);
    expect(isReviewedMigrationFile('190_phoenix_inter_org_alert_cqrs_boundary.sql')).toBe(true);
    expect(isReviewedMigrationFile('191_phoenix_canonical_scope_topology_read_contract.sql')).toBe(true);
    expect(isReviewedMigrationFile('192_phoenix_anonymous_read_surface_convergence.sql')).toBe(true);
    expect(isReviewedMigrationFile('193_phoenix_inter_org_alert_command_surface_hardening.sql')).toBe(true);
    expect(isReviewedMigrationFile('194_phoenix_authorization_surface_reproducibility_convergence.sql')).toBe(true);
    expect(isReviewedMigrationFile('195_phoenix_auth_helper_profile_schema_qualification.sql')).toBe(true);
    expect(isReviewedMigrationFile('196_phoenix_secdef_relation_schema_qualification.sql')).toBe(true);
    expect(isReviewedMigrationFile('197_phoenix_public_execute_convergence.sql')).toBe(true);
    expect(isReviewedMigrationFile('198_phoenix_secdef_search_path_convergence.sql')).toBe(true);
    expect(isReviewedMigrationFile('199_phoenix_command_center_read_contract.sql')).toBe(true);
    expect(isReviewedMigrationFile('200_phoenix_demo_purge_auth_boundary_correction.sql')).toBe(true);
    expect(isReviewedMigrationFile('201_phoenix_organization_archive_dependency_guard.sql')).toBe(true);
    expect(isReviewedMigrationFile('202_phoenix_organization_archive_reciprocal_guard.sql')).toBe(true);
    expect(isReviewedMigrationFile('203_phoenix_material_dispensing_suspension.sql')).toBe(true);
    expect(isReviewedMigrationFile('204_phoenix_dispensing_suspension_enforcement_dispense.sql')).toBe(true);
    expect(isReviewedMigrationFile('205_phoenix_dispensing_suspension_enforcement_fefo.sql')).toBe(true);
    expect(isReviewedMigrationFile('206_phoenix_dispensing_suspension_enforcement_suggestions.sql')).toBe(true);
    expect(isReviewedMigrationFile('207_phoenix_dispensing_suspension_enforcement_warehouse_send.sql')).toBe(true);
    ...
    expect(isReviewedMigrationFile('212_phoenix_central_needs_need_lines.sql')).toBe(true);
    ...
    expect(isReviewedMigrationFile('217_phoenix_central_needs_c5_safety_convergence.sql')).toBe(true);
  ...
  it('future and alternate names remain fail-closed',()=>{
    // M218 UNAUTHORIZED PROBE: reviewing 217 authorizes 217 by exact filename
    ...
    expect(extractMigrationNumber(SYNTH_NEXT)).toBe(getNextUnreviewedMigrationNumber());
    expect(isReviewedMigrationFile(SYNTH_NEXT)).toBe(false);
    expect(reviewedMigrationFilesAbove(getMaximumReviewedMigrationNumber())).toEqual([]);
