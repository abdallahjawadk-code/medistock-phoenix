...
'188_phoenix_public_qr_facility_context.sql','189_phoenix_inter_org_alert_canonical_identity.sql','190_phoenix_inter_org_alert_cqrs_boundary.sql','191_phoenix_canonical_scope_topology_read_contract.sql','192_phoenix_anonymous_read_surface_convergence.sql',
]);

// G4.1 / Inter-Org Alert CQRS Boundary: 190 is REAL; the unknown boundary
// moves to 191. It must stay a name that is genuinely absent from the manifest,
// otherwise the fail-closed assertions below would be asserting nothing.
const SYNTH_NEXT='193_unreviewed_test_migration.sql';
const SYNTH_ALT='187_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 192',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(192);
  ...
  it('pins maximum 192 and next unreviewed 193',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(192);
    expect(getNextUnreviewedMigrationNumber()).toBe(193);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=190;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,181],[150,181],[170,181],[174,181],[175,181],[176,181],[177,181],[178,181],[179,181],[180,181],[181,181],[182,182],[183,183],[184,184],[185,185],[186,186],[187,187]] as const)
