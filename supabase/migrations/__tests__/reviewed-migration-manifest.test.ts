...
//
// FIXED-NEAR-EXPIRY-073-A: migration 073 is now genuinely reviewed and
// registered, so the "next unreviewed number" synthetic moved 073 → 074.
...
const SYNTH_NEXT = '077_unreviewed_test_migration.sql';
const SYNTH_NEXT_ALT = '077_phoenix_some_other_name.sql';
...
describe('3. reviewed maximum derives from the registry', () => {
  it('the current reviewed maximum is 76', () => {
    expect(getMaximumReviewedMigrationNumber()).toBe(76);
  });

  it('the next unreviewed number is 77', () => {
    expect(getNextUnreviewedMigrationNumber()).toBe(77);
  });
...
describe('4. migrations 059–073 registered by exact name; 074 absent', () => {
  ...
  it('registers exactly one each of migrations 074/075/076 (no alternate names)', () => {
  ...
  it('does not register migration 077 in any form', () => {
    const seventySevens = REVIEWED_MIGRATION_FILES.filter(f => extractMigrationNumber(f) === 77);
    expect(seventySevens).toEqual([]);
  });
});
...
describe('5. approval requires exact filename membership, nothing less', () => {
  it('rejects a synthetic unreviewed migration 077 (the next unreviewed number)', () => {
...
describe('6. derived slices remain exact-filename lists', () => {
  it('reviewedMigrationFilesAbove(43) yields the exact 044–076 filenames', () => {
  ...
  it('reviewedMigrationFilesAbove(64) contains exactly migrations 065-076', () => {
    expect(reviewedMigrationFilesAbove(64)).toEqual([REAL_065, REAL_066, REAL_067, REAL_068, REAL_069, REAL_070, REAL_071, REAL_072, REAL_073, REAL_074, REAL_075, REAL_076]);
  });

  it('reviewedMigrationFilesAbove(71) contains exactly migrations 072-076', () => {
    expect(reviewedMigrationFilesAbove(71)).toEqual([REAL_072, REAL_073, REAL_074, REAL_075, REAL_076]);
  });

  it('reviewedMigrationFilesAbove(72) contains exactly migrations 073-076', () => {
    expect(reviewedMigrationFilesAbove(72)).toEqual([REAL_073, REAL_074, REAL_075, REAL_076]);
  });

  it('reviewedMigrationFilesAbove(76) is empty (nothing beyond the ceiling)', () => {
    expect(reviewedMigrationFilesAbove(76)).toEqual([]);
  });
