...
const SYNTH_NEXT='188_unreviewed_test_migration.sql';
const SYNTH_ALT='187_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 187',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(187);
  ...
  it('pins maximum 187 and next unreviewed 188',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(187);
    expect(getNextUnreviewedMigrationNumber()).toBe(188);
