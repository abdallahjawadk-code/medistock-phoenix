...
'202_phoenix_organization_archive_reciprocal_guard.sql','203_phoenix_material_dispensing_suspension.sql','204_phoenix_dispensing_suspension_enforcement_dispense.sql','205_phoenix_dispensing_suspension_enforcement_fefo.sql','206_phoenix_dispensing_suspension_enforcement_suggestions.sql','207_phoenix_dispensing_suspension_enforcement_warehouse_send.sql',
]);

// MDS-203..207: 207 is now the reviewed warehouse-send suspension enforcement;
// the synthetic unknown boundary moves to 208 and must remain genuinely absent.
const SYNTH_NEXT='208_unreviewed_test_migration.sql';
const SYNTH_ALT='187_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 207',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(207);
  ...
  it('pins maximum 207 and next unreviewed 208',()=>{
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=207;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
