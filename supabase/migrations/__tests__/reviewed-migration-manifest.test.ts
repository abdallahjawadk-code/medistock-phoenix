...
const SYNTH_NEXT='186_unreviewed_test_migration.sql';
const SYNTH_ALT='185_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 185',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(185);
  ...
  it('pins maximum 185 and next unreviewed 186',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(185);
    expect(getNextUnreviewedMigrationNumber()).toBe(186);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=186;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,181],[150,181],[170,181],[174,181],[175,181],[176,181],[177,181],[178,181],[179,181],[180,181],[181,181],[182,182],[183,183],[184,184],[185,185],[186,186]] as const)
