...
const SYNTH_NEXT='180_unreviewed_test_migration.sql';
const SYNTH_ALT='179_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 179',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(179);
  ...
  it('pins maximum 179 and next unreviewed 180',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(179);
    expect(getNextUnreviewedMigrationNumber()).toBe(180);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=180;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,179],[150,179],[170,179],[174,179],[175,179],[176,179],[177,179],[178,179],[179,179],[180,180]] as const)
