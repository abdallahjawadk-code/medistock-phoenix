...
describe('reviewed migration manifest through 180',()=>{
  ...
  it('pins maximum 180 and next unreviewed 181',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(180);
    expect(getNextUnreviewedMigrationNumber()).toBe(181);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=181;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,180],[150,180],[170,180],[174,180],[175,180],[176,180],[177,180],[178,180],[179,180],[180,180],[181,181]] as const)
