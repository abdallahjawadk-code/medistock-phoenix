...
// Independent exact-filename counter-gate. Never derive this list from disk or
// REVIEWED_MIGRATION_FILES: both must independently match it.
const EXPECTED: readonly string[] = Object.freeze([
  ...
  '171_phoenix_organization_kind_pharmacy_department_authority.sql','172_phoenix_patient_dispensing_contract.sql','173_phoenix_database_security_surface_hardening.sql','174_phoenix_authenticated_rpc_surface_hardening.sql','175_phoenix_read_helper_anonymous_surface_hardening.sql',
]);

const SYNTH_NEXT='176_unreviewed_test_migration.sql';
const SYNTH_ALT='175_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 175',()=>{
  ...
  it('pins maximum 175 and next unreviewed 176',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(175);
    expect(getNextUnreviewedMigrationNumber()).toBe(176);
    expect(isReviewedMigrationFile('175_phoenix_read_helper_anonymous_surface_hardening.sql')).toBe(true);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=176;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,175],[150,175],[170,175],[174,175],[175,175],[176,176]] as const)
