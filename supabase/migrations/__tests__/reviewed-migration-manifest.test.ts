...
// H/U4: 195 is now REAL (auth-helper schema qualification); the unknown
// boundary moves to 196. It must stay a name that is genuinely absent from the
// manifest, otherwise the fail-closed assertions below would assert nothing.
const SYNTH_NEXT='196_unreviewed_test_migration.sql';
const SYNTH_ALT='187_phoenix_some_other_name.sql';

describe('reviewed migration manifest through 195',()=>{
  it('counter-gates registry and disk by exact filename',()=>{
    expect(EXPECTED).toHaveLength(195);
  ...
  it('pins maximum 195 and next unreviewed 196',()=>{
