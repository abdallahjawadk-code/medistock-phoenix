...
describe('reviewed migration manifest through 177',()=>{
  ...
  it('pins maximum 177 and next unreviewed 178',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(177);
    expect(getNextUnreviewedMigrationNumber()).toBe(178);
    expect(isReviewedMigrationFile('177_phoenix_canonical_public_qr.sql')).toBe(true);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=178;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,177],[150,177],[170,177],[174,177],[175,177],[176,177],[177,177],[178,178]] as const)
