...
const SYNTH_NEXT = '074_unreviewed_test_migration.sql';
const SYNTH_NEXT_ALT = '074_phoenix_some_other_name.sql';
...
describe('3. reviewed maximum derives from the registry', () => {
  it('the current reviewed maximum is 73', () => {
    expect(getMaximumReviewedMigrationNumber()).toBe(73);
  });

  it('the next unreviewed number is 74', () => {
    expect(getNextUnreviewedMigrationNumber()).toBe(74);
  });
...
describe('4. migrations 059–073 registered by exact name; 074 absent', () => {
  ...
  it('does not register migration 074 in any form', () => {
    const seventyFours = REVIEWED_MIGRATION_FILES.filter(f => extractMigrationNumber(f) === 74);
    expect(seventyFours).toEqual([]);
  });
});
...
describe('5. approval requires exact filename membership, nothing less', () => {
  it('rejects a synthetic unreviewed migration 074 (the next unreviewed number)', () => {
...
describe('6. derived slices remain exact-filename lists', () => {
  it('reviewedMigrationFilesAbove(43) yields the exact 044–073 filenames', () => {
  ...
  it('reviewedMigrationFilesAbove(64) contains exactly migrations 065-073', () => {
    expect(reviewedMigrationFilesAbove(64)).toEqual([REAL_065, REAL_066, REAL_067, REAL_068, REAL_069, REAL_070, REAL_071, REAL_072, REAL_073]);
  });

  it('reviewedMigrationFilesAbove(71) contains exactly migrations 072-073', () => {
    expect(reviewedMigrationFilesAbove(71)).toEqual([REAL_072, REAL_073]);
  });

  it('reviewedMigrationFilesAbove(72) contains exactly migration 073', () => {
    expect(reviewedMigrationFilesAbove(72)).toEqual([REAL_073]);
  });

  it('reviewedMigrationFilesAbove(73) is empty (nothing beyond the ceiling)', () => {
    expect(reviewedMigrationFilesAbove(73)).toEqual([]);
  });
