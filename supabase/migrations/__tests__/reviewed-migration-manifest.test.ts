...
const SYNTH_060 = '060_unreviewed_test_migration.sql';
...
describe('3. reviewed maximum derives from the registry', () => {
  it('the current reviewed maximum is 59', () => {
    expect(getMaximumReviewedMigrationNumber()).toBe(59);
  });

  it('the next unreviewed number is 60', () => {
    expect(getNextUnreviewedMigrationNumber()).toBe(60);
  });
...
// ============================================================================
// 4. Migration 059 is registered by its exact real name; 060 is not registered
// ============================================================================

describe('4. migration 059 registered by exact name; 060 absent', () => {
...
describe('5. approval requires exact filename membership, nothing less', () => {
  it('rejects a synthetic unreviewed migration 060', () => {
    expect(isReviewedMigrationFile(SYNTH_060)).toBe(false);
    expect(findUnreviewedMigrationFiles([...actualSqlFiles(), SYNTH_060])).toEqual([SYNTH_060]);
  });
...
describe('6. derived slices remain exact-filename lists', () => {
  it('reviewedMigrationFilesAbove(43) yields the exact 044–059 filenames', () => {
  ...
  it('reviewedMigrationFilesAbove(59) is empty (nothing beyond the ceiling)', () => {
    expect(reviewedMigrationFilesAbove(59)).toEqual([]);
  });
  ...
  it('reviewedMigrationFilesBetween is inclusive and exact', () => {
    expect(reviewedMigrationFilesBetween(58, 59)).toEqual([
      '058_phoenix_public_qr_dosage_form.sql',
      '059_phoenix_public_qr_concentration.sql',
    ]);
  });
...
describe('7. future-migration workflow', () => {
  it('Scenario A — unreviewed 060 on disk fails validation', () => {
    const disk = [...actualSqlFiles(), SYNTH_060];
    const unreviewed = findUnreviewedMigrationFiles(disk);
    expect(unreviewed).toEqual([SYNTH_060]);
    // "validation fails" = the manifest assertion this test file makes would fail.
    expect(unreviewed.length).toBeGreaterThan(0);
  });

  it('Scenario B — explicitly reviewing 060 accepts that exact name only', () => {
    // In-memory registry copy: what maintainers will do for real, once.
    const nextRegistry = new Set([...REVIEWED_MIGRATION_FILES, SYNTH_060]);
    const isReviewedNext = (f: string): boolean => nextRegistry.has(f);

    expect(isReviewedNext(SYNTH_060)).toBe(true); // the reviewed name is accepted
  ...
  it('adding a disk file without registering it fails (registry is the gate)', () => {
    expect(findUnreviewedMigrationFiles([...actualSqlFiles(), SYNTH_060])).not.toEqual([]);
  });

  it('registering a name without shipping the file fails (disk is the counter-gate)', () => {
    const pretendRegistry = [...REVIEWED_MIGRATION_FILES, SYNTH_060];
    const actual = new Set(actualSqlFiles());
    expect(pretendRegistry.filter(f => !actual.has(f))).toEqual([SYNTH_060]);
  });
});
...
describe('8. registry purity', () => {
  it('is frozen (cannot be mutated at runtime by a test)', () => {
    ...
    expect(() => {
      (REVIEWED_MIGRATION_FILES as string[]).push(SYNTH_060);
    }).toThrow();
    expect(isReviewedMigrationFile(SYNTH_060)).toBe(false);
  });
