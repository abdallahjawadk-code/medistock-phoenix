...
describe('reviewed migration manifest through 181',()=>{
  ...
  it('pins maximum 181 and next unreviewed 182',()=>{
    expect(getMaximumReviewedMigrationNumber()).toBe(181);
    expect(getNextUnreviewedMigrationNumber()).toBe(182);
  ...
  it('derived above/between slices remain exact against the independent list',()=>{
    ...
    for(let n=0;n<=182;n++) expect(reviewedMigrationFilesAbove(n),`above ${n}`).toEqual(EXPECTED.filter(f=>num(f)>n));
    for(const [a,b] of [[1,181],[150,181],[170,181],[174,181],[175,181],[176,181],[177,181],[178,181],[179,181],[180,181],[181,181],[182,182]] as const)
