...
describe('189 · registration and file hygiene', () => {
  ...
  it('is followed by exactly 190 through 202, the new ceiling, and 203 stays absent', () => {
    ...
    const NEXT_20 = '209_phoenix_central_needs_registry.sql';
    const NEXT_21 = '210_phoenix_central_needs_workflow_rpcs.sql';
    const NEXT_22 = '211_phoenix_central_needs_batch_and_disposition.sql';
    const NEXT_23 = '212_phoenix_central_needs_need_lines.sql';
    // CN-2B / Finding-1 corrective (213): the Central Needs beneficiary-
    // column-mapping layer. It is now the reviewed successor.
    const NEXT_24 = '213_phoenix_central_needs_beneficiary_column_mapping.sql';
    ...
    // search_path change. It adds no alert-identity SQL, so what 189 asserts is
    // unaffected; it is listed to keep this successor guard exhaustive.
    const NEXT_25 = '214_phoenix_central_needs_review_readiness_volatility.sql';
    ...
    // helper, NOT VALID source-value CHECK, approval-gate fence) — no alert-identity SQL,
    // so what this suite asserts is unaffected; listed to keep this successor guard
    // exhaustive.
    const NEXT_28 = '217_phoenix_central_needs_c5_safety_convergence.sql';
    ...
    expect(REVIEWED_MIGRATION_FILES.slice(i + 1)).toEqual([NEXT, NEXT_2, NEXT_3, NEXT_4, NEXT_5, NEXT_6, NEXT_7, NEXT_8, NEXT_9, NEXT_10, NEXT_11, NEXT_12, NEXT_13, NEXT_14, NEXT_15, NEXT_16, NEXT_17, NEXT_18, NEXT_19, NEXT_20, NEXT_21, NEXT_22, NEXT_23, NEXT_24, NEXT_25, NEXT_26, NEXT_27, NEXT_28]);
    ...
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^209_/.test(f))).toEqual([NEXT_20]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^210_/.test(f))).toEqual([NEXT_21]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^211_/.test(f))).toEqual([NEXT_22]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^212_/.test(f))).toEqual([NEXT_23]);
