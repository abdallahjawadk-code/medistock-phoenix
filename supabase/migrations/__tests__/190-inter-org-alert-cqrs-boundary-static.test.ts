...
describe('190 · registration and file hygiene', () => {
  ...
  it('is followed by 191 through 202, the current ceiling, and 203 stays absent', () => {
    ...
    const NEXT_11 = '201_phoenix_organization_archive_dependency_guard.sql';
    const NEXT_12 = '202_phoenix_organization_archive_reciprocal_guard.sql';
    const NEXT_13 = '203_phoenix_material_dispensing_suspension.sql';
    const NEXT_14 = '204_phoenix_dispensing_suspension_enforcement_dispense.sql';
    ...
    const NEXT_19 = '209_phoenix_central_needs_registry.sql';
    const NEXT_20 = '210_phoenix_central_needs_workflow_rpcs.sql';
    const NEXT_21 = '211_phoenix_central_needs_batch_and_disposition.sql';
    const NEXT_22 = '212_phoenix_central_needs_need_lines.sql';
    // CN-2B / Finding-1 corrective (213): the Central Needs beneficiary-
    // column-mapping layer. It is now the reviewed successor.
    const NEXT_23 = '213_phoenix_central_needs_beneficiary_column_mapping.sql';
    ...
    // search_path change. It adds no alert command/query SQL, so what 190
    // asserts is unaffected; listed to keep this successor guard exhaustive.
    const NEXT_24 = '214_phoenix_central_needs_review_readiness_volatility.sql';
    // C2/M215: the governed Central Needs correction lifecycle — function-only, and no
    // alert command/query SQL, so what this suite asserts is unaffected; listed to keep this
    // successor guard exhaustive.
    const NEXT_25 = '215_phoenix_central_needs_governed_correction_lifecycle.sql';
    // C4/M216: the additive, independently reviewed Central Needs beneficiary-region
    // persistence (region version relation, governed M213 <-> region coexistence and
    // conversion) — no alert command/query SQL, so what this suite asserts is unaffected;
    // listed to keep this successor guard exhaustive.
    const NEXT_26 = '216_phoenix_central_needs_region_persistence.sql';
    // C5/M217: the Central Needs safety convergence (classifier, shared quantity-lineage
    // helper, NOT VALID source-value CHECK, approval-gate fence) — no alert command/query
    // SQL, so what this suite asserts is unaffected; listed to keep this successor guard
    // exhaustive.
    const NEXT_27 = '217_phoenix_central_needs_c5_safety_convergence.sql';
    expect(REVIEWED_MIGRATION_FILES.filter(f => f.startsWith('212_'))).toEqual([NEXT_22]);
    ...
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^199_/.test(f))).toEqual([NEXT_9]);
  });
});
