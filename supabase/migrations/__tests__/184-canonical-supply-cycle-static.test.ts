...
describe('184 · registration and file hygiene', () => {
  ...
  // used to play.
  it('is followed by exactly 185 through 202, with 202 highest reviewed', () => {
    ...
    const NEXT_24 = '208_phoenix_dispensing_suspension_enforcement_replenishment_and_drafts.sql';
    const NEXT_25 = '209_phoenix_central_needs_registry.sql';
    const NEXT_26 = '210_phoenix_central_needs_workflow_rpcs.sql';
    const NEXT_27 = '211_phoenix_central_needs_batch_and_disposition.sql';
    const NEXT_28 = '212_phoenix_central_needs_need_lines.sql';
    // CN-2B / Finding-1 corrective (213): the Central Needs beneficiary-
    // column-mapping layer. It is now the reviewed successor.
    const NEXT_29 = '213_phoenix_central_needs_beneficiary_column_mapping.sql';
    // M214: the Central Needs readiness-RPC transaction-mode correction — that
    // one function's volatility (STABLE -> VOLATILE), no body, grant or
    // search_path change. It adds no supply-cycle SQL, so what 184 asserts is
    // unaffected; it is listed to keep this successor guard exhaustive.
    const NEXT_30 = '214_phoenix_central_needs_review_readiness_volatility.sql';
    // C2/M215: the governed Central Needs correction lifecycle — function-only, and no
    // supply-cycle SQL, so what this suite asserts is unaffected; listed to keep this
    // successor guard exhaustive.
    const NEXT_31 = '215_phoenix_central_needs_governed_correction_lifecycle.sql';
    // C4/M216: the additive Central Needs beneficiary-region persistence (region version
    // relation, governed M213 <-> region coexistence/conversion) — no supply-cycle SQL,
    // so what this suite asserts is unaffected; listed to keep this successor guard
    // exhaustive.
    const NEXT_32 = '216_phoenix_central_needs_region_persistence.sql';
    // C5/M217: the Central Needs safety convergence (classifier, shared quantity-lineage
    // helper, NOT VALID source-value CHECK, approval-gate fence) — no supply-cycle SQL,
    // so what this suite asserts is unaffected; listed to keep this successor guard
    // exhaustive.
    const NEXT_33 = '217_phoenix_central_needs_c5_safety_convergence.sql';
    ...
    expect(REVIEWED_MIGRATION_FILES.filter(f => f.startsWith('208_'))).toEqual([NEXT_24]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => f.startsWith('209_'))).toEqual([NEXT_25]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => f.startsWith('210_'))).toEqual([NEXT_26]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => f.startsWith('211_'))).toEqual([NEXT_27]);
    ...
    expect(REVIEWED_MIGRATION_FILES.slice(i + 1)).toEqual([NEXT, NEXT_2, NEXT_3, NEXT_4, NEXT_5, NEXT_6, NEXT_7, NEXT_8, NEXT_9, NEXT_10, NEXT_11, NEXT_12, NEXT_13, NEXT_14, NEXT_15, NEXT_16, NEXT_17, NEXT_18, NEXT_19, NEXT_20, NEXT_21, NEXT_22, NEXT_23, NEXT_24, NEXT_25, NEXT_26, NEXT_27, NEXT_28, NEXT_29, NEXT_30, NEXT_31, NEXT_32, NEXT_33]);
    expect(REVIEWED_MIGRATION_FILES[REVIEWED_MIGRATION_FILES.length - 1]).toBe(NEXT_33);
    ...
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^216_/.test(f))).toEqual([NEXT_32]);
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^217_/.test(f))).toEqual([NEXT_33]);
    // The next unauthorized number must still fail closed — now 218.
    expect(REVIEWED_MIGRATION_FILES.filter(f => /^218_/.test(f))).toHaveLength(0);
  });
