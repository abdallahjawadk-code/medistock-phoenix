...
describe('18/19. Does not modify frontend production files or package/lockfiles', () => {
  it('no frontend production file has a working-tree diff from this phase', () => {
    ...
    //
    // FRONTEND-LIVE-REMOVED-AT-FILTERS-A: src/features/institutions/InstitutionScreen.tsx
    // is also excluded — a later, separately-reviewed phase (after migration
    // 053 was applied) that replaced PortAvailabilitySection's blunt
  ...
  // PHASE3-DEEP-CLEAN-AVAILABILITY-DATA-A: UserManagementScreen.tsx now has a
  // diff — a later, separately-reviewed phase that additively wires in the
  // Super Admin-only AvailabilityCleanupWizard at the end of that screen.
  // Every other diff line beyond that one import + render is still disallowed.
  ...
  it('the StatusCenterScreen.tsx diff (from the later SAFE-PROFESSIONAL-XLSX-EXPORT-A, PHASE2-STATUS-CENTER-ENTERED-PRICE-FILTER-XLSX-A, and PHASE2-STATUS-CENTER-OUTLET-REPORT-MODAL-A phases) only touches the CSV-to-XLSX export replacement / entered-price filter / outlet-report-modal wiring, never this migration\'s own concerns', () => {
    let diff = '';
    try {
      diff = execSync(
    ...
    if (diff.trim()) {
      expect(diff).toMatch(/exportAvailabilityXlsx|removed_at|enteredPrice|priceFilterMode|OutletAvailabilityReportModal|outletOptions/);
      expect(diff).not.toMatch(/service_role|auth\.admin/);
      expect(diff).not.toMatch(/graph\.facebook\.com|access_token=|api\.whatsapp\.com|Bearer |sendMessage/i);
      expect(diff).not.toMatch(/source_expiry_risk_tier|source_expiry_days_remaining/);
      expect(diff).not.toMatch(/CREATE (OR REPLACE )?FUNCTION|supabase\.rpc\(/);
...
describe('20. Does not touch QR/export/print/user-management/auth/session files', () => {
  ...
  // PHASE3-DEEP-CLEAN-AVAILABILITY-DATA-A — see the dedicated guard above.
  it('QR and auth service files are untouched', () => {
    let diff = '';
    try {
      diff = execSync(
...
describe('22. Migration review: only exactly-reviewed migrations exist; any unregistered migration still fails', () => {
