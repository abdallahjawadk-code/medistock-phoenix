...
BEGIN
  ...
  SELECT d.organization_id INTO v_point_org
  ...
  IF NOT FOUND OR NOT (v_role='super_admin' OR (v_org IS NOT NULL AND v_org=v_point_org)) THEN
    RETURN v_empty;
  END IF;
  ...
  WITH canonical AS (
    SELECT
      ...
      MAX(s.trade_name) AS trade_name,
    ...
    SELECT
      c.organization_id AS canonical_org_id,
      c.distribution_point_id AS canonical_point_id,
      c.scientific_name AS canonical_scientific_name,
      c.concentration_key AS canonical_concentration_key,
      c.dosage_form_key AS canonical_dosage_form_key,
      c.national_code_key AS canonical_national_code_key,
      c.batch_number_key AS canonical_batch_number_key,
      c.expiry_date_key AS canonical_expiry_date_key,
      c.internal_batch_reference_key AS canonical_internal_ref_key,
      c.trade_name AS canonical_trade_name,
      c.unit_price AS canonical_unit_price,
      c.updated_at AS canonical_updated_at,
      c.on_hand_quantity,
      c.available_quantity,
      ia.id AS catalogue_id,
      ia.local_item_id,ia.organization_id AS catalogue_org_id,ia.port_name,
    ...
    SELECT
      j.catalogue_id AS id,
      j.catalogue_id AS catalogue_item_availability_id,
      CASE WHEN j.catalogue_id IS NOT NULL
        THEN 'catalogue:'||j.catalogue_id::text
        ELSE 'stock:'||md5(concat_ws('|',
          p_distribution_point_id::text,j.canonical_scientific_name,
          j.canonical_concentration_key,j.canonical_dosage_form_key,j.canonical_national_code_key,
          j.canonical_batch_number_key,j.canonical_expiry_date_key::text,j.canonical_internal_ref_key))
      END AS row_key,
      j.local_item_id,
      p_distribution_point_id AS distribution_point_id,
      ...
      public.phoenix_derive_outlet_availability_condition(
        COALESCE(j.available_quantity,0),COALESCE(j.catalogue_expiry_date,NULLIF(j.canonical_expiry_date_key,DATE '0001-01-01'))
      ) AS condition,
      ...
      j.notes,COALESCE(j.catalogue_updated_at,j.canonical_updated_at) AS updated_at,
      j.port_name,j.supply_type,j.removed_at,
      ...
      CASE WHEN public.phoenix_derive_outlet_availability_condition(
        COALESCE(j.available_quantity,0),COALESCE(j.catalogue_expiry_date,NULLIF(j.canonical_expiry_date_key,DATE '0001-01-01'))
      ) IN ('expired','missing') THEN 0 ELSE COALESCE(j.available_quantity,0) END AS canonical_usable_quantity,
      CASE WHEN li.id IS NULL THEN NULL ELSE jsonb_build_object(
        'id',li.id,'local_code',li.local_code,
        'central_items',CASE WHEN ci.id IS NULL THEN NULL ELSE jsonb_build_object(
          'id',ci.id,'name',ci.name,'name_ar',ci.name_ar,'unit',ci.unit,'barcode',ci.barcode
        ) END
      ) END AS local_items
    FROM joined j
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'quantity',s.quantity,'condition',s.condition,
    'batch_number',s.batch_number,'national_code',s.national_code,'expiry_date',s.expiry_date,
    'notes',s.notes,'updated_at',s.updated_at,'port_name',s.port_name,'supply_type',s.supply_type,
    'removed_at',s.removed_at,'scientific_name',s.scientific_name,'trade_name',s.trade_name,
    'dosage_form',s.dosage_form,'concentration',s.concentration,'price',s.price,
  ...
  RETURN jsonb_build_object('ok',true,'scope','distribution_point',
    'distribution_point_id',p_distribution_point_id,'source','canonical_outlet_stock','rows',v_rows);
END;
$function$;
...
COMMENT ON FUNCTION public.phoenix_outlet_availability_read_model(uuid) IS
  'STAGE-G-G1 / 176: canonical-first authenticated outlet availability CQRS. '
  'Physical quantity/condition derive only from outlet_stock. item_availability is optional '
  'catalogue/visibility metadata. canonical-only rows remain visible with a NULL catalogue id; '
  'row_key is display identity only and must never be passed to visibility writers.';
...
BEGIN
  ...
  END IF;
  IF NOT has_function_privilege('anon','public.get_public_qr_payload(text)'::regprocedure,'EXECUTE') THEN
