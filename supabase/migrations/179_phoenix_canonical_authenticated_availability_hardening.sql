...
--
-- ENCODING: the stock row_key hashes a jsonb ARRAY, not concat_ws. A
-- delimiter-joined string is only unambiguous if no component can contain the
-- delimiter, and batch_number / internal_batch_reference are free text that can
-- contain '|' and date-like substrings. Measured on the rig, these two tuples
-- collide under concat_ws and stay distinct under jsonb:
...
-- The jsonb encoding is chosen so this row identity does not silently depend on
-- that cross-column CHECK continuing to hold — it quotes and escapes every
-- element and is injective on its own terms. It costs nothing and removes the
-- whole class.
--
...
BEGIN
  ...
  WITH canonical AS (
    ...
    SELECT j.catalogue_id AS id,j.catalogue_id AS catalogue_item_availability_id,
      ...
      -- row with no physical stock at all falls back to the catalogue form.
      -- jsonb (not concat_ws) because free-text batch/internal-ref values may
      -- contain the separator — see this file's header for the measured
      -- collision.
      CASE WHEN j.canonical_material_identity_key IS NOT NULL
        THEN 'stock:'||md5(jsonb_build_array(
               p_distribution_point_id::text,
               j.canonical_material_identity_key,
               j.canonical_batch_number_key,
               j.canonical_expiry_date_key::text,
               j.canonical_internal_ref_key)::text)
...
COMMENT ON FUNCTION public.phoenix_outlet_availability_read_model(uuid) IS
  'STAGE-G-G3.1 (179): authenticated outlet availability, grouped by the canonical outlet_stock.material_identity_key (Migration 150) instead of raw material columns, so unit-distinct identities such as 5 box and 3 strip can never be summed into 8. Publishes an additive row-level `unit` taken from the canonical identity the row represents; NULL stays NULL and central_items.unit is never a fallback in either direction. Stock-backed rows carry a row_key derived from that physical identity (jsonb-encoded, because free-text batch/internal-ref values may contain the separator); catalogue-only rows keep the catalogue form. Physical quantity and condition still come only from canonical outlet_stock — item_availability remains optional metadata and never becomes stock truth. Signature, every prior response key, SECURITY DEFINER, search_path, organization scoping and the anon revocation are unchanged.';
