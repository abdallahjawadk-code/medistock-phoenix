...
-- is a SEPARATE physical model ... tracked as a follow-up migration." 070 is
-- that migration:
--
...
-- return authorization path. 070 makes it load-bearing FOR THIS DOMAIN ONLY:
...
--
-- ─────────────────────────────────────────────────────────────────────────────
-- DUAL PROVENANCE, XOR: A DISPATCH LINE IS NOT THE ONLY LEGITIMATE ORIGIN
-- ─────────────────────────────────────────────────────────────────────────────
-- 069's return lines name a specific PRIOR RECEIPT (warehouse_transfer_lines),
-- because 068's forward SEND/RECEIVE actually produces rows there. The outlet
-- domain is different: as of 069, NO RPC anywhere sends a warehouse_dispatch
-- (061 shipped the schema only; 062/067/068/069 never built the dispatch
-- SEND/CREATE path — confirmed by grep across every migration). Outlet stock
-- that exists today therefore did NOT arrive via a warehouse_dispatch_line in
-- practice; it arrived via a plain 'add' movement (manual/legacy entry) on
-- outlet_stock_movements. Requiring original_dispatch_line_id NOT NULL would
-- make 070 unusable against real data. So a return line's provenance is an
-- XOR of two possibilities, each structurally proven, never RPC-trusted:
--   (a) original_dispatch_line_id  -> warehouse_dispatch_lines.id, the
--       dispatch-schema path, for whenever migration 06x eventually ships a
--       real dispatch SEND;
--   (b) original_inbound_movement_id -> outlet_stock_movements.id, restricted
--       BY COMPOSITE FK (not a CHECK guessing at another table's column) to
--       movement_type IN ('dispatch_receive', 'add') on THE EXACT outlet_stock
--       row being returned from. 'dispense'/'correction'/'subtract'/
--       'reserve'/'release'/'set_exact' can never be named — a return can
--       never be provenanced by an outbound consumption event. This is what
--       "legacy dispensing is rejected until reclassified" means concretely:
--       a batch whose only paper trail is a dispense is not eligible
--       provenance at all, not even indirectly.
-- Exactly one of the two must be set (wrrl-style XOR CHECK), never both,
-- never neither.
--
-- ─────────────────────────────────────────────────────────────────────────────
...
-- migration created the row. 070 reuses it verbatim for outlet returns
...
--     unused, in outlet_stock_movements_type_chk since 067) and 'add' (already
--     in warehouse_stock_movements_type_chk since 060) for the two debit/
--     credit legs, exactly as 069 reused 'dispatch_return'/'add'.
...
BEGIN
  IF to_regclass('public.distribution_points') IS NULL
    ...
    RAISE EXCEPTION 'ABORT 070: expected 060/061/067/069 schema is absent. Apply earlier migrations first.';
  END IF;
  ...
  IF to_regprocedure('public.phoenix_profile_has_scoped_permission(uuid,text,uuid,uuid,uuid)') IS NULL
    ...
    RAISE EXCEPTION 'ABORT 070: 062/067 helper RPCs are absent.';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'outlet_stock_movements_type_chk') THEN
    RAISE EXCEPTION 'ABORT 070: outlet_stock_movements_type_chk (067) is absent.';
  END IF;
...
-- Bounded by on_hand_delta: only 'dispatch_receive'/'add' movements (always
-- delta > 0) are eligible provenance in the first place (enforced by the FK
-- in section 5), so this cap is meaningful for exactly the rows that can ever
-- be named.
DO $$ BEGIN
  ALTER TABLE public.outlet_stock_movements
...
-- Composite FK targets for the movement-provenance path. All trivially
-- unique (id is PK) — each pins one extra column so the FK can prove identity,
-- organization and TYPE ELIGIBILITY structurally, not by RPC discipline.
...
COMMENT ON TABLE public.outlet_return_requests IS
  'OUTLET-TO-INSTITUTION-RETURN-070-A: either the outlet (requested_by_side='
...
-- ============================================================================
-- 5. outlet_return_request_lines — dual provenance, XOR, fail-closed reasons
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.outlet_return_request_lines (
  ...
  -- Dual provenance, XOR — see file header. Exactly one of the two names the
  -- original inbound event this line is returning from.
  original_dispatch_line_id    uuid REFERENCES public.warehouse_dispatch_lines(id) ON DELETE RESTRICT,
  original_inbound_movement_id uuid REFERENCES public.outlet_stock_movements(id) ON DELETE RESTRICT,
  -- Snapshotted so the FK in section 6 can prove type-eligibility structurally
  -- (see outlet_stock_movements_id_type_uniq above) rather than trusting the
  -- RPC to have checked movement_type itself.
  original_inbound_movement_type text,
  ...
  -- XOR: exactly one provenance, never both, never neither.
  CONSTRAINT orrl_provenance_xor_chk
    CHECK (
      (original_dispatch_line_id IS NOT NULL AND original_inbound_movement_id IS NULL
        AND original_inbound_movement_type IS NULL)
      OR
      (original_dispatch_line_id IS NULL AND original_inbound_movement_id IS NOT NULL
        AND original_inbound_movement_type IS NOT NULL)
    ),

  -- Type eligibility, structural: the referenced movement's ACTUAL
  -- movement_type must equal the snapshot, and the snapshot is restricted to
  -- the two credit types a return may legitimately trace to. A 'dispense' or
  -- 'correction' provenance is not just discouraged — it cannot satisfy this
  -- FK at all.
  CONSTRAINT orrl_inbound_movement_type_eligible_chk
    CHECK (original_inbound_movement_type IS NULL
           OR original_inbound_movement_type IN ('dispatch_receive', 'add')),
...
-- A given prior receipt (either provenance) can be named at most once per
-- return request — ambiguity prevention, same reasoning as 069.
CREATE UNIQUE INDEX IF NOT EXISTS orrl_request_dispatch_line_uniq
  ...
  WHERE original_dispatch_line_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS orrl_request_inbound_movement_uniq
  ...
  WHERE original_inbound_movement_id IS NOT NULL;
...
COMMENT ON TABLE public.outlet_return_request_lines IS
  'OUTLET-TO-INSTITUTION-RETURN-070-A: dual provenance, XOR — either a '
  'warehouse_dispatch_lines row (once a real dispatch SEND ships) or an '
  'eligible outlet_stock_movements row (dispatch_receive/add only — never '
  'dispense/correction/subtract/reserve/release/set_exact). Bounded at the RPC '
  'level by that provenance''s own remaining returnable quantity.';
...
COMMENT ON TABLE public.outlet_return_shipments IS
  'OUTLET-TO-INSTITUTION-RETURN-070-A: one outlet->institution return '
...
CREATE TABLE IF NOT EXISTS public.outlet_return_shipment_lines (
  ...
  -- Carried from the request line at ADD-LINE time — same XOR discipline.
  original_dispatch_line_id       uuid REFERENCES public.warehouse_dispatch_lines(id) ON DELETE RESTRICT,
  original_inbound_movement_id     uuid REFERENCES public.outlet_stock_movements(id) ON DELETE RESTRICT,
  ...
  CONSTRAINT orsl_provenance_xor_chk
    CHECK (
      (original_dispatch_line_id IS NOT NULL AND original_inbound_movement_id IS NULL)
      OR
      (original_dispatch_line_id IS NULL AND original_inbound_movement_id IS NOT NULL)
...
COMMENT ON TABLE public.outlet_return_shipment_lines IS
  'OUTLET-TO-INSTITUTION-RETURN-070-A: one returned batch per line, dual '
  'provenance XOR, literal conservation equation, no expiry-refusal check at '
  'send (mirrors 069''s named divergence). RECEIVE classification is '
  'fail-closed, identical policy to 069.';
...
-- 10c. ADD LINE (draft only) — validates the XOR provenance and its cap.
CREATE OR REPLACE FUNCTION public.phoenix_add_outlet_return_request_line(
  ...
  p_original_inbound_movement_id uuid DEFAULT NULL,
...
BEGIN
  ...
  IF (p_original_dispatch_line_id IS NOT NULL) = (p_original_inbound_movement_id IS NOT NULL) THEN
    RAISE EXCEPTION 'exactly_one_provenance_required' USING ERRCODE = '23514';
  END IF;
  ...
  IF p_original_dispatch_line_id IS NOT NULL THEN
    ...
    END IF;
    IF v_dispatch.resulting_outlet_stock_id IS NULL THEN
      RAISE EXCEPTION 'original_dispatch_line_not_yet_resulted_in_outlet_stock' USING ERRCODE = '23514';
    END IF;
    SELECT * INTO v_stock
  ...
  ELSE
    -- Movement provenance: eligibility (dispatch_receive/add) is proven by the
    -- composite FK at INSERT time, not re-checked here — but organization and
    -- outlet must still be verified against the LOCKED movement row.
    SELECT * INTO v_movement
    FROM public.outlet_stock_movements WHERE id = p_original_inbound_movement_id FOR UPDATE;
    IF NOT FOUND OR v_movement.organization_id <> v_request.source_organization_id THEN
      RAISE EXCEPTION 'original_inbound_movement_not_found' USING ERRCODE = 'P0002';
    END IF;
    IF v_movement.movement_type NOT IN ('dispatch_receive', 'add') THEN
      RAISE EXCEPTION 'original_inbound_movement_not_eligible' USING ERRCODE = '23514';
    END IF;
    IF v_movement.distribution_point_id <> v_request.distribution_point_id THEN
      RAISE EXCEPTION 'original_inbound_movement_not_at_this_outlet' USING ERRCODE = '23514';
    END IF;

    SELECT * INTO v_stock
    FROM public.outlet_stock WHERE id = v_movement.outlet_stock_id;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'source_outlet_stock_not_found' USING ERRCODE = 'P0002';
    END IF;

    v_cap := v_movement.on_hand_delta - v_movement.returned_quantity;
    IF p_requested_quantity > v_cap THEN
      RAISE EXCEPTION 'requested_quantity_exceeds_returnable_cap' USING ERRCODE = '23514';
    END IF;
    ...
    INSERT INTO public.outlet_return_request_lines (
      return_request_id, source_organization_id,
      original_inbound_movement_id, original_inbound_movement_type,
      scientific_name, concentration, dosage_form, unit, national_code,
      batch_number, internal_batch_reference, expiry_date,
      reason_code, reason_text, requested_quantity
    ) VALUES (
      p_return_request_id, v_request.source_organization_id,
      p_original_inbound_movement_id, v_movement.movement_type,
      v_stock.scientific_name, v_stock.concentration, v_stock.dosage_form, v_stock.unit,
      v_stock.national_code, v_stock.batch_number, v_stock.internal_batch_reference, v_stock.expiry_date,
      p_reason_code, v_reason_text, p_requested_quantity
    )
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'original_inbound_movement_id', p_original_inbound_movement_id,
...
BEGIN
  ...
  -- Resolve the SOURCE outlet_stock row from the SAME provenance the line
  -- was created with, re-checked live under this lock.
  IF v_line.original_dispatch_line_id IS NOT NULL THEN
    SELECT s.* INTO v_stock FROM public.outlet_stock s
    JOIN public.warehouse_dispatch_lines d ON d.resulting_outlet_stock_id = s.id
    WHERE d.id = v_line.original_dispatch_line_id
    FOR UPDATE OF s;
  ELSE
    SELECT s.* INTO v_stock FROM public.outlet_stock s
    JOIN public.outlet_stock_movements m ON m.outlet_stock_id = s.id
    WHERE m.id = v_line.original_inbound_movement_id
    FOR UPDATE OF s;
  END IF;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'source_outlet_stock_not_found' USING ERRCODE = 'P0002';
  ...
  -- Increment whichever provenance cap this line actually used.
  IF v_line.original_dispatch_line_id IS NOT NULL THEN
...
BEGIN
  ...
  -- Lock whichever provenance cap this line used, exactly mirroring SEND.
  IF v_line.original_dispatch_line_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    PERFORM 1 FROM public.outlet_stock_movements
    WHERE id = v_line.original_inbound_movement_id FOR UPDATE;
  END IF;
  ...
  IF v_line.original_dispatch_line_id IS NOT NULL THEN
    UPDATE public.warehouse_dispatch_lines
     ...
     WHERE id = v_line.original_dispatch_line_id;
  END IF;
  -- (The movement-provenance path has no equivalent "received so far" counter
  -- to increment — outlet_stock_movements.returned_quantity already tracks
  -- SENT, not received; there is no destination-side analogue on that row
  -- because a movement row belongs to the SOURCE outlet, not the
  -- destination warehouse. return_request_line's own fulfilled_quantity,
  -- already incremented at SEND, remains the authoritative committed figure
  -- for both provenance paths.)
...
REVOKE ALL ON FUNCTION public.phoenix_add_outlet_return_request_line(uuid, uuid, uuid, integer, text, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.phoenix_add_outlet_return_request_line(uuid, uuid, uuid, integer, text, text)
  TO authenticated;
...
COMMENT ON FUNCTION public.phoenix_receive_outlet_return_shipment_line(uuid, uuid, integer, text, text, text) IS
  'OUTLET-TO-INSTITUTION-RETURN-070-A: idempotent receipt at the institution '
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'VERIFY FAILED (070): an unexpected CASCADE exists on a return line table';
  END IF;
  ...
  -- exactly their pre-070 vocabulary; 'return_send'/'add' are REUSED, not new).
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'outlet_stock_movements_type_chk'
      AND pg_get_constraintdef(oid) LIKE '%''return_send''%'
  ) THEN
    RAISE EXCEPTION 'VERIFY FAILED (070): outlet_stock_movements_type_chk missing return_send';
  END IF;

  -- 14c. Structural quantity guards exist.
  FOREACH v_def IN ARRAY ARRAY[
    'orrl_provenance_xor_chk', 'orsl_provenance_xor_chk', 'orsl_conservation_eq_chk',
    'orrl_inbound_movement_type_eligible_chk', 'wdl_returned_qty_chk', 'osm_returned_qty_chk'
  ] LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = v_def) THEN
      RAISE EXCEPTION 'ABORT 070: missing structural guard: %', v_def;
    END IF;
  END LOOP;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'outlet_stock_movements_return_once_uniq') THEN
    RAISE EXCEPTION 'ABORT 070: outlet return-send idempotency index missing';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'warehouse_stock_movements_outlet_return_once_uniq') THEN
    RAISE EXCEPTION 'ABORT 070: outlet return-receive idempotency index missing';
  END IF;
  ...
  FOREACH v_def IN ARRAY ARRAY[
    ...
    ASSERT v_body LIKE '%''' || v_def || '''%',
      'VERIFY FAILED (070): return-receive is missing the mandatory-quarantine reason: ' || v_def;
  END LOOP;
  ASSERT v_body LIKE '%v_reason_code IS NULL%',
    'VERIFY FAILED (070): a NULL reason_code is not treated as mandatory quarantine';
  ASSERT v_body LIKE '%return_receive_requires_explicit_disposition_decision%',
    'VERIFY FAILED (070): near_expiry/excess/shipment_error do not require an explicit decision';
  ASSERT v_body NOT LIKE '%v_disposition := ''restockable'';%',
    'VERIFY FAILED (070): restockable must never be a hardcoded default';
  ...
  ASSERT pg_get_functiondef(
    ...
    'VERIFY FAILED (070): return-send must not refuse expired batches';

  -- 14g. A rejection never touches either balance and is exception_pending.
  ASSERT pg_get_functiondef(
    'public.phoenix_receive_outlet_return_shipment_line(uuid,uuid,integer,text,text,text)'::regprocedure)
    LIKE '%custody_state = ''exception_pending''%',
    'VERIFY FAILED (070): a rejected return is not tracked as exception_pending';
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'VERIFY FAILED (070): authenticated holds a direct return write privilege';
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.role_table_grants
    WHERE table_schema = 'public'
    ...
    RAISE EXCEPTION 'VERIFY FAILED (070): anon can read outlet return data';
  END IF;
  ...
  BEGIN
    ASSERT v_qr_def NOT ILIKE '%outlet_return%' AND v_qr_def NOT ILIKE '%quarantine%',
      'VERIFY FAILED (070): outlet return or quarantine data leaked into public QR';
  END;
  ...
  FOREACH v_def IN ARRAY ARRAY[
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'ABORT 070: RLS not enabled on %', v_def;
    END IF;
  END LOOP;

  RAISE NOTICE '070 verified: outlet->institution return reuses distribution_points.'
    'warehouse_id as the structural pairing (no new route table), dual XOR '
    'provenance (dispatch line or eligible inbound movement), fail-closed '
    'disposition identical to 069, literal conservation equation, and zero '
    'direct client write path on any new table. NOT APPLIED by this PR.';
END;
$verify$;
