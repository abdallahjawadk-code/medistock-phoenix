...
describe('A7.2.4 preservation and fail-closed boundaries',()=>{
  ...
  it('has no unapproved migration/service/Auth/RBAC/environment diff',()=>{
    ...
    const EXCLUDED:string[]=[
      'src/app/AppContext.tsx','src/app/AuthenticatedApp.tsx','src/app/__tests__/auth-resilience-context.runtime.test.tsx','src/app/__tests__/auth-dead-end-screens.runtime.test.tsx','src/app/__tests__/auth-session-race.runtime.test.tsx','src/app/__tests__/db-pressure-quick-wins.test.ts','src/app/__tests__/auth-cold-start-deadline.runtime.test.tsx','src/app/screen-continuity.ts','src/app/__tests__/phase-b45-screen-continuity.runtime.test.ts','src/shared/supabase/services/auth.service.ts','src/shared/supabase/services/availability.service.ts','src/shared/supabase/services/__tests__/frontend-live-removed-at-filters.test.ts','src/shared/supabase/services/__tests__/auth-signout.runtime.test.ts','src/shared/supabase/__tests__/permission-persistence.test.ts','src/shared/supabase/__tests__/permission-matrix-readiness.test.ts','src/shared/supabase/__tests__/permission-save-diagnostics.test.ts','src/shared/authz/__tests__/screen-access.test.ts',
      ...
      // enforcement migrations, plus one dynamic test per enforcement point
      // (203's domain RPCs, and 204-208's enforcement of dispense, FEFO,
      // suggestions, guarded warehouse sends, and replenishment/drafts).
      // Registered by EXACT filename, exactly as M201/M202 were. A7.2.4's own
      // subject is untouched by any of them, and no other file under
      // supabase/ or src/shared/supabase changed.
      ...
      'supabase/migrations/__tests__/205-dispensing-suspension-enforcement-fefo.dynamic.test.ts',
      ...
      // CN-2B conformance (212): the Central Needs operational need-line layer
      // — one reviewed, forward-only migration plus its static and dynamic
      // proofs. It adds no UI, no branding asset, no RLS policy on an A7.2.4
      // surface and no authorization decision: its own RLS lives on two NEW
      // tables and authorizes through the existing
      // phoenix_status_center_authorized helper, so A7.2.4's own subject is
      // untouched by it. Registered by EXACT filename, exactly as
      // M209/M210/M211 were. No wildcard and no directory exemption, so every
      // other file under supabase/ still fails this guard closed.
