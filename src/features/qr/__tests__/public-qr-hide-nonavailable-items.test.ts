...
describe('QR-HIDE-NONAVAILABLE-ITEMS-FROM-PUBLIC-LIST-A: safety guards', () => {
  ...
  it('no migration SQL file has a working-tree diff (other than the already-approved 053 removed_by-FK fix)', () => {
    ...
    try {
      diff = execSync(
        'git diff -- "supabase/migrations/*.sql" ":!supabase/migrations/053_item_availability_removed_marker.sql" ":!supabase/migrations/054_dashboard_condition_counts_rpcs.sql" ":!supabase/migrations/055_phoenix_clean_availability_data.sql"',
  ...
  it('no migration 056 (or higher) file was created (053/054/055 are later, separately-reviewed DB-REMOVED-OUTLET-MATERIAL-MARKER-053-A / PHASE2-DASHBOARD-PERFORMANCE-RPCS-054-A / PHASE3-DEEP-CLEAN-AVAILABILITY-DATA-A additions)', () => {
    const migsDir = join(ROOT, 'supabase/migrations');
    const matches = (readdirSync(migsDir) as string[]).filter(f => /^0(5[6-9]|[6-9][0-9])_/.test(f));
