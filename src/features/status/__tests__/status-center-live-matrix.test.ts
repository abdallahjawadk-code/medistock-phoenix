...
describe('StatusCenter: print and export', () => {
  ...
  it('print HTML includes a footer with the "generated by" brand line', () => {
    expect(screen).toContain('report_footer_generated_by');
  });

  it('expiry column is normalized through formatStableDate (not a raw passthrough)', () => {
    expect(screen).toContain('function expiryDisplay');
    const fn = screen.slice(screen.indexOf('function expiryDisplay'), screen.indexOf('function expiryDisplay') + 300);
    expect(fn).toContain('formatStableDate(r.expiry_date, lang)');
  });
